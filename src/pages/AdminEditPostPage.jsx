...
export default function AdminEditPostPage() {
  ...
  const processFiles = (files) => {
    ...
    if (validImages.length + images.length + existingImages.length > 20) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Erreur lors de la mise à jour de la publication');
      console.error('Erreur:', err);
    } finally {
      setLoading(false);
    }
  ...
  if (fetching) {
    return (
      <AdminLayout pageTitle="Modifier une Publication">
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h2 className="text-3xl font-bold text-gray-900">Modifier la Publication</h2>
          </div>
          <p className="text-gray-600 ml-15">Mettez à jour le contenu et les images</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white/40 backdrop-blur-xl rounded-3xl p-8 border border-gray-200/40 shadow-sm space-y-8">
              ...
              >
                ...
                <label htmlFor="file-input" className="cursor-pointer block">
                  ...
                  <p className="text-gray-600 text-sm">
                    JPG, PNG, GIF, WebP — {existingImages.length + images.length}/20 images
                  </p>
                </label>
              </div>
            </div>
          ...
          {existingImages.length > 0 && (
            <div className="border-t border-gray-200/40 pt-8">
              <h3 className="text-lg font-bold text-gray-900 flex items-center gap-2 mb-4">
                <div className="w-1 h-6 bg-gradient-to-b from-teal-600 to-teal-400 rounded"></div>
                Photos existantes <span className="ml-2 px-3 py-1 bg-teal-100/60 text-teal-700 text-sm font-bold rounded-full border border-teal-200/40">({existingImages.length})</span>
              </h3>
                ...
                {existingImages.map((img) => (
                  <div key={img.id} className="relative group">
                    <img
                      ...
                      className="w-full h-32 object-cover rounded-2xl border border-gray-200/40 shadow-sm"
                    />
                    <button
                      type="button"
          ...
          {images.length > 0 && (
            <div className="border-t border-gray-200/40 pt-8">
              <h3 className="text-lg font-bold text-gray-900 flex items-center gap-2 mb-4">
                <div className="w-1 h-6 bg-gradient-to-b from-teal-600 to-teal-400 rounded"></div>
                Nouvelles photos <span className="ml-2 px-3 py-1 bg-teal-100/60 text-teal-700 text-sm font-bold rounded-full border border-teal-200/40">({images.length})</span>
              </h3>
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
